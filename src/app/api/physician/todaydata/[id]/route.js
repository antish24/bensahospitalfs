...
export const GET = async (request, { params }) => {
  ...
  try {
    ...
    const appointments = await Appointment.find({physicianId:user._id,createdAt: {$gte: today}});
    const assigned = await AssignDoc.find({physician:id,createdAt: {$gte: today}});
    const diagnosticRequest = await Diagnostic.find({physicianId:user._id,createdAt: {$gte: today}});
